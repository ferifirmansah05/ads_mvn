        ...
        for cab in all_cab:
            for date in all_date:
                ...
                if 'GOJEK' in all_kat:
                    ...
                    all.to_csv(f'1. ABO/_final/GOJEK/{cab}/GOJEK_{cab}_{date}.csv', index=False)
                ...
                if 'QRIS SHOPEE' in all_kat:
                    ...
                    all.loc[all[~all['HELP'].str.contains('Bayar')].index,'HELP'] = all.loc[all[~all['HELP'].astype(str).str.contains('Bayar')].index,'KET'].apply(lambda x: label_1(x))
                    all.loc[all[~all['HELP'].str.contains('Bayar')].index,'KET'] = all.loc[all[~all['HELP'].astype(str).str.contains('Bayar')].index,'KET'].apply(lambda x:x if (('Balance' in x)) else '')
                    all['DATE'] = all['DATE'].dt.strftime('%d/%m/%Y')
                    all['TIME'] = all['TIME'].dt.strftime('%H:%M:%S')
                ...
                if 'GRAB' in all_kat:
                    ...
                    all['DATE'] = all['DATE'].dt.strftime('%d/%m/%Y')
                    all['TIME'] = all['TIME'].dt.strftime('%H:%M:%S')
                    all.to_csv(f'1. ABO/_final/GRAB/{cab}/GRAB_{cab}_{date}.csv', index=False)
                ...
                if 'SHOPEEPAY' in all_kat:
                    ...
                    all['DATE'] = all['DATE'].dt.strftime('%d/%m/%Y')
                    all['TIME'] = all['TIME'].dt.strftime('%H:%M:%S')
                    all.to_csv(f'1. ABO/_final/SHOPEEPAY/{cab}/SHOPEEPAY_{cab}_{date}.csv', index=False)
                ...
                if not ((dfinv[((dfinv['KAT']  ==  "QRIS ESB") | (dfinv['KAT']  ==  "QRIS ESB ORDER")) & (dfinv['CAB']  ==  cab) & (dfinv['DATE']==date)].empty) or
                    ...
                    all['TIME'] = all['TIME'].dt.strftime('%H:%M:%S')
                    all['KAT'] = 'QRIS ESB'
                    all.to_csv(f'1. ABO/_final/QRIS ESB/{cab}/QRIS ESB_{cab}_{date}.csv', index=False)
                ...
                if not ((dfinv[(dfinv['KAT']  ==  "QRIS Telkom") & (dfinv['CAB']  ==  cab) & (dfinv['DATE']==date)].empty) or
                    ...
                    all['TIME'] = all['TIME'].dt.strftime('%H:%M:%S')
                    all['KAT'] = 'QRIS TELKOM'
                    all.to_csv(f'1. ABO/_final/QRIS TELKOM/{cab}/QRIS TELKOM_{cab}_{date}.csv', index=False)
